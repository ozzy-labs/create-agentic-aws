...
```text
base → typescript → python → amazon-q → claude-code → copilot → cdk → terraform → vpc → lambda → ecs → eks → ec2 → s3 → dynamodb → aurora → rds → sqs → sns → eventbridge → step-functions → api-gateway → cloudfront → cognito → cloudwatch
```
